...
RectModel2::RectModel2(vec3 size) : PhysicalModel(), SolidModel(GL_TRIANGLES, 36), mBreakable(false)
{
...
//								{ vec3( x,-y, 1.0+z), vec3( 0.0f, 0.0f, 1.0f), vec3(0.0f, 1.0f, 0.0f) },
                                // right - purple
        						{ vec3( x, y, 1.0+z), vec3( 1.0f, 0.0f, 0.0f), vec3(1.0f, 0.0f, 1.0f) },
        						{ vec3( x,-y,-1.0-z), vec3( 1.0f, 0.0f, 0.0f), vec3(1.0f, 0.0f, 1.0f) },
        						{ vec3( x, y,-1.0-z), vec3( 1.0f, 0.0f, 0.0f), vec3(1.0f, 0.0f, 1.0f) },
...
RectModel2::~RectModel2()
{
}

// TODO getBoxDef*S*
q3BoxDef RectModel2::GetBoxDef(){
	q3BoxDef def;
...
void RectModel2::Update(float dt)
{
...
// some threshold
void RectModel2::handleBeginContact(const q3ContactConstraint *contact){
...
//		bool sub_breakable = glm::dot(size, size) > 0.2;
		...
		if (true/*sub_breakable*/){
			...
			for (int x = 0; x < 2; ++x){
				for (int y = 0; y < 2; ++y){
					for (int z = 0; z < 2; ++z){
						RectModel2 *shard = new RectModel2();
...
void RectModel2::SetBreakable(bool breakable){
	mBreakable = breakable;
}

bool RectModel2::IsBreakable(){
	return mBreakable;
}


bool RectModel2::ParseLine(const std::vector<ci_string> &token)
{
    if (token.empty())
    {
        return true;
